...
/**
 * Reaches an enclosed upper deck whose only entrance is an eight-block ladder.
 */
public final class PathLadderAscentScenario extends TestScenario {
    ...
    @Override
    public String name() {
        return "path-ladder-ascent";
    }

    @Override
    public String description() {
        return "Climb the only entrance to an enclosed upper deck";
    }
    ...
    public void stage(TestArena arena) {
        ...
        // The deck is a closed room. Its east wall supports the ladder; its floor stops one block
        // before that wall, so climbing to y=8 gives a real step onto the deck rather than a
        // ladder buried in its floor.
    ...
    public void start(TestArena arena) {
        this.goalPos = arena.at(2, 8, 0);
        this.goal = new GoalBlock(this.goalPos);
        arena.note("goal is %s, inside a deck whose only entrance is the ladder at x+7", this.goalPos);
    ...
    public Verdict poll(TestArena arena, int elapsedTicks) {
        ...
        if (this.goal.isInGoal(feet)) {
            return this.ladderTicks > 0
                    ? Verdict.pass(String.format("climbed the ladder and reached the deck in %d ticks", elapsedTicks))
                    : Verdict.fail("reached the upper goal without ever occupying its only ladder");
        }
