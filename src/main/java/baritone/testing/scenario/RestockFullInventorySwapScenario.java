...
/** A full inventory must make room for a fetched material without losing protected items. */
public final class RestockFullInventorySwapScenario extends TestScenario {

    private static final int BUILD_X = 6;
    private static final int BOX_X = 2;
    private static final int BOX_Z = 4;
    private static final int JUNK_SLOTS = 33;
    private static final int SOURCE_COUNT = 64;

    @Override
    public String name() {
        return "restock-full-inventory-swap";
    }

    @Override
    public String description() {
        return "Make room in a full inventory for a fetched build material without losing protected items";
    }
    ...
    public void start(TestArena arena) {
        ...
        if (world == null) {
            arena.note("no world data, so the full-inventory box at %s could not be registered", box);
            return;
        }
    ...
    public Verdict poll(TestArena arena, int elapsedTicks) {
        ...
        if (!RestockInventoryFixture.protectedItemsIntact(arena)) {
            return Verdict.fail("the full-inventory build completed, but the iron pickaxe, sword, food, or helmet changed");
        }
        if (source != SOURCE_COUNT - 1 || dumped <= 0) {
            return Verdict.fail("the full-inventory build completed with source white concrete=%d and dumped red concrete=%d; expected %d and some dumped junk",
                    source, dumped, SOURCE_COUNT - 1);
        }
        return Verdict.pass("a full inventory made room by depositing junk, fetched one white concrete, and retained protected items");
    }
