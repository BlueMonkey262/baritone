...
/** An ignored block requested as air must remain, while neighboring work still proceeds. */
public final class BuilderIgnoreBlocksAirScenario extends TestScenario {

    private static final int TARGET_X = 5;

    @Override
    public String name() {
        return "builder-ignore-blocks-air";
    }

    @Override
    public String description() {
        return "Leave a configured ignored block in place while building a neighboring target";
    }
    ...
    public Map<String, Object> settings() {
        ...
        settings.put("buildIgnoreBlocks", List.of(Blocks.STONE));
        return settings;
    }
    ...
    public void start(TestArena arena) {
        arena.note("air target at x+%d ignores existing stone; neighboring concrete target is x+%d",
                TARGET_X, TARGET_X + 2);
    ...
    public Verdict poll(TestArena arena, int elapsedTicks) {
        boolean ignoredIntact = arena.stateAt(TARGET_X, 0, 0).is(Blocks.STONE);
        boolean targetBuilt = arena.stateAt(TARGET_X + 2, 0, 0).is(Blocks.WHITE_CONCRETE);
        arena.note("builder evidence at t=%d: ignored=%s, neighbor=%s, player=%s",
        ...
        if (!ignoredIntact) {
            return Verdict.fail("buildIgnoreBlocks did not preserve the stone requested as air");
        }
        return targetBuilt
                ? Verdict.pass("preserved the ignored air target and placed the neighboring concrete")
                : null;
    }

    @Override
    public String timeoutDiagnosis(TestArena arena) {
        return "ignored=" + arena.stateAt(TARGET_X, 0, 0).getBlock()
